...
class Etsy(object):
    ...
    def URIModification(self, Call):
        URI = self.MethodsDict[Call]['URI']
        URIMatch = re.findall(r"/:\w*", URI)
        ReqedParams = []
        if URIMatch:
            for Required in URIMatch:
                ReqedParams.append(str(Required).replace("/:", ""))
        return ReqedParams
    ...
    def GetInfo(self, Call):
        ...
        ReqedParams = self.URIModification(Call)
        for Each in Method:
            if Each in ListDisplay:
                try:
                    for item in Method[Each]:
                        if item in ReqedParams:
                            print("%s List: %s: (%s)(*Required*)") % (Each, 
                                item, Method[Each][item])
                        else:
                            print("%s List:  %s: (%s)") % (Each,
                                item, Method[Each][item])               
                except(TypeError):
                    print("%s : None") % Each
            else:
    ...
    def writeFunction(self, Call):
    
        # Function variables to write
        Method = self.MethodsDict[Call]
    

        FunctionDefString = """
def %s(self, param=None):""" % Call
    

        URIString = '%s' % Method['URI']
        ParametersString = '%s' % Method['Parameters']
        VisibilityString = '%s' % Method['Visibility']
        HTTP_MethodString = '%s' % Method['HTTP_Method']
        DefaultsString = '%s' % Method['Defaults']
        TypeString = '%s' % Method['Type']
        DescriptionString = '%s' % Method['Description']
        NameString = '%s' % Method['Name']
...
Visibility       %s
Parameters       %s
URI              %s""") % (NameString, DescriptionString, TypeString, DefaultsString, 
                HTTP_MethodString, VisibilityString, ParametersString, URIString)

        


        print FunctionDefString    
        print Documentation
